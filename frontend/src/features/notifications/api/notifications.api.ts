...
export async function getUnreadNotificationsCount(): Promise<number> {
  const result = await NotificationsService.getUnreadNotificationsCountApiV1NotificationsUnreadCountGet()
  return (result as { count: number }).count ?? 0
}

export async function markNotificationAsRead(notificationId: string): Promise<NotificationRead> {

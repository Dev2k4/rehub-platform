...
async def create_direct_order(
	...
	await crud_notification.create_notification(
		...
		message=f"You have successfully placed an order for '{listing.title}'.",
...
async def complete_order(
	...
	if escrow:
		raise HTTPException(
			status_code=400,
			detail="Đơn hàng này sử dụng Ví Rehub. Vui lòng sử dụng luồng xác nhận qua Ví Rehub.",
		)
...
async def cancel_order(
	...
	if escrow and escrow.status != EscrowStatus.AWAITING_FUNDING:
		raise HTTPException(
			status_code=400,
			detail="Không thể hủy đơn hàng đã thanh toán qua Ví Rehub. Vui lòng mở khiếu nại hoặc liên hệ admin.",
		)

	updated_order = await crud_order.cancel_order(db, order)
	
	# Get listing to include its title in the notification
	listing = await crud_order.get_listing_for_order(db, order.listing_id)
	listing_title = listing.title if listing else "your listing"
	...
	await crud_notification.create_notification(
		...
		message=f"The order for '{listing_title}' was cancelled by the {('buyer' if current_user.id == order.buyer_id else 'seller')}.",
		data={"order_id": str(order.id), "listing_id": str(order.listing_id)},
	)
	await _broadcast_order_event(updated_order, "order:status_changed")
	return updated_order
